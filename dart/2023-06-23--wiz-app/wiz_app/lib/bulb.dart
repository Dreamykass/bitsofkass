...
class BulbManager extends ChangeNotifier {
  ...
  void sendCommandGetState() {
    ...
    var addressStr = prefsGetBulbAddresses().first;
    var address = InternetAddress(addressStr); // '192.168.0.165'
    var port = 38899;
  ...
  void sendCommand(String command) {
    ...
    for (var addressStr in prefsGetBulbAddresses()) {
      var encoded = utf8.encode(command);
      var address = InternetAddress(addressStr);
      var port = 38899;
      socket?.send(encoded, address, port);
    }
