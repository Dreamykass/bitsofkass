...
#define OBX_VERSION_PATCH \
    0 // values >= 100 are reserved for dev releases leading to the next minor/major increase
...
obx_err obx_txn_abort(
    OBX_txn *txn); // Internal note: will make more sense once we introduce obx_txn_reset
...
obx_err obx_cursor_put4(
    OBX_cursor *cursor, obx_id id, const void *data, size_t size, OBXPutMode mode);
