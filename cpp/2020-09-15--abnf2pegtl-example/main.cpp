...
namespace TAO_PEGTL_NAMESPACE {
  namespace abnf {
    ...
    namespace {
      ...
      rules_t::reverse_iterator find_rule(
        rules_t& r,
        const std::string& v,
        const rules_t::reverse_iterator& rbegin) {
        return std::find_if(
          rbegin, r.rend(), [&](const rules_t::value_type& p) {
            return TAO_PEGTL_STRCASECMP(p.c_str(), v.c_str()) == 0;
          });
      }
    ...
    struct selector
      : parse_tree::selector<
          Rule,
          parse_tree::store_content::on<grammar::rulename,
                                        grammar::prose_val,
          ...
          parse_tree::remove_content::on<grammar::option,
                                         grammar::and_predicate,
                                         grammar::not_predicate,
                                         grammar::rule>,
          parse_tree::fold_one::on<grammar::alternation,
                                   grammar::group,
                                   grammar::repetition,
    ...
    std::string to_string_unwrap_seq(const node_ptr& n) {
      if (n->is_type<grammar::group>() ||
          n->is_type<grammar::concatenation>()) {
    ...
    namespace {
      ...
      std::string get_rulename(const node_ptr& n,
                               const bool print_forward_declarations) {
        ...
        if (keywords.count(v) != 0 || v.find("__") != std::string::npos) {
          throw parse_error('\'' + n->string() + "' is a reserved rulename",
                            n->begin());
        }
        if (print_forward_declarations &&
            find_rule(rules_defined, v) != rules_defined.rend()) {
          std::cout << "struct " << v << ";\n";
        }
        rules.push_back(v);
        return v;
      ...
      std::string gen_val(const node_ptr& n) {
        if (n->children.size() == 2) {
          if (n->children.back()->is_type<T>()) {
            return prefix + "range< " + to_string(n->children.front()) + ", " +
                   to_string(n->children.back()->children.front()) + " >";
          }
        }
      ...
      static void transform(node_ptr& n) {
        ...
        if (op == "=") {
          if (!previous_rules.try_emplace(rname, n.get()).second) {
            throw parse_error("rule '" + rname + "' is already defined",
                              n->begin());
          }
        }
        // if it is an "incremental alternation", we need to consolidate the
        // assigned alternations
        else if (op == "=/") {
          const auto p = previous_rules.find(rname);
          if (p == previous_rules.end()) {
            throw parse_error("incremental alternation '" + rname +
                                "' without previous rule definition",
                              n->begin());
          }
          auto& previous = p->second->children.back();

          // if the previous rule does not assign an alternation, create an
          // intermediate alternation and move its assignee into it.
          ...
          // append all new options to the previous rule's assignee (which
          // always is an alternation now)
          previous->m_end = n->children.back()->m_end;

          // if the new rule itself contains an alternation, append the
          // individual entries...
          ...
          throw parse_error("invalid operator '" + op +
    ...
    stringifier make_stringifier() {
      ...
      nrv.default_ = [](const node_ptr& n) -> std::string {
        throw parse_error("missing to_string() for " + std::string(n->type),
                          n->begin());
      };

      nrv.add<grammar::rulename>(
        [](const node_ptr& n) { return get_rulename(n, true); });

      nrv.add<grammar::rule>([](const node_ptr& n) {
        return "struct " + get_rulename(n->children.front(), false) + " : " +
               to_string(n->children.back()) + " {};";
      });
      ...
      nrv.add<grammar::hex_val::value>(
        [](const node_ptr& n) { return "0x" + n->string(); });
      nrv.add<grammar::dec_val::value>(
        [](const node_ptr& n) { return n->string(); });
      ...
      nrv.add<grammar::hex_val::type>(
        [](const node_ptr& n) { return gen_val<grammar::hex_val::range>(n); });
      nrv.add<grammar::dec_val::type>(
        [](const node_ptr& n) { return gen_val<grammar::dec_val::range>(n); });
      nrv.add<grammar::bin_val::type>(
        [](const node_ptr& n) { return gen_val<grammar::bin_val::range>(n); });

      nrv.add<grammar::alternation>([](const node_ptr& n) {
        return prefix + "sor< " + to_string(n->children) + " >";
      });
      nrv.add<grammar::option>([](const node_ptr& n) {
        return prefix + "opt< " + to_string(n->children) + " >";
      });
      nrv.add<grammar::group>([](const node_ptr& n) {
        return prefix + "seq< " + to_string(n->children) + " >";
      });

      nrv.add<grammar::prose_val>(
        [](const node_ptr& n) { return "/* " + n->string() + " */"; });
      ...
      nrv.add<grammar::repetition>([](const node_ptr& n) -> std::string {
        ...
        if ((star != rep.size() - 1) && max.empty()) {
          throw parse_error("repetition maximum of zero not allowed",
                            n->begin());
        }
        ...
        if (min_val > max_val) {
          throw parse_error("repetition minimum which is greater than the "
                            "repetition maximum not allowed",
                            n->begin());
        }
        ...
        auto min_element = (min_val == 1)
                             ? content
                             : (prefix + "rep< " + min + ", " + content + " >");
        if (min_val == max_val) {
          return min_element;
        }
